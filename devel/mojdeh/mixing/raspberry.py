...
class DatafileGenerator():
    ...
    def setUpSimulation(self):
        ...
        #vertex1 = (xlen*0.04,zlen*0.95)
        #vertex2 = (xlen*0.04,zlen*0.45)
        #vertex3 = (xlen*0.08,zlen*0.25)
        #vertex4 = (xlen*0.08,zlen*0.20)
        
        #vertex5 = (xlen*0.16,zlen*0.95)
        #vertex6 = (xlen*0.16,zlen*0.45)
        #vertex7 = (xlen*0.12,zlen*0.25)
        #vertex8 = (xlen*0.12,zlen*0.20)
        ...
        #elf.drawWallFromVtxs(vertex2,vertex3)
        ...
        #self.fillCubeWithCBDVtxs(vertex1,vertex2)
            ...
            for yi in np.arange(yl,yh,self.dia):
                for zi in np.arange(zl,zh,self.dia):
                    ...
                    for line in activeParticleLines:
                        data = line.split()
                        distance_sq = (xi - float(data[3]))**2 + (yi - float(data[4]))**2 + (zi - float(data[5]))**2
    ...
    def drawSphere(self,x,y,z,radius):
        self.molID += 1
        for xi in np.arange(x-radius,x+radius,self.dia/2):
            for yi in np.arange(y-radius,y+radius,self.dia/2):
                for zi in np.arange(z-radius,z+radius,self.dia/2):
                    if ((xi-x)**2 + (yi-y)**2 + (zi-z)**2) < radius**1.2:
                        self.appendLine(self.active_type,xi,yi,zi)
    ...
    def drawDiParticle(self,x,y,z,radius):
        self.molID += 1
