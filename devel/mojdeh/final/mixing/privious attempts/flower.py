...
class DatafileGenerator():
    ...
    def setUpSimulation(self):
        ...
        #self.fillCubeWithCBDVtxs(vertex3,vertex4,checkForOverlap=True)  # this will give us the number of active particles
            ...
            for yi in np.arange(y-radius,y+radius,self.dia):
                for zi in np.arange(z-radius,z+radius,self.dia):
                    if ((xi-x)**2 + (yi-y)**2 + (zi-z)**2) < radius**1.2:
                        #if ((xi-x)**2 + (yi-y)**2 + (zi-z)**2) > radius**.5:  # making hollow
                            self.appendLine(self.active_type,xi,yi,zi)
