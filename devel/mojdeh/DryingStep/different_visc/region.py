...
class DatafileGenerator():
    ...
    def setUpSimulation(self):
        xlen = (self.x1-self.x0)/3

        self.fillCube(self.x0,self.y0,self.z0,xlen,self.y1,self.z1,1)
        self.fillCube(xlen,self.y0,self.z0,2*xlen,self.y1,self.z1,2)
        self.fillCube(2*xlen,self.y0,self.z0,self.x1,self.y1,self.z1,3)

    def fillCube(self,x,y,z,x2,y2,z2,atom_type):
        xl = min(x,x2)
        xh = max(x,x2)
        yl = min(y,y2)
        yh = max(y,y2)
