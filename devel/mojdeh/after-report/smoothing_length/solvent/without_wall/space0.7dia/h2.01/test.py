...
class DatafileGenerator():
   ...
   # data string containing atom ID, molecule ID, type, rho, e, cv, x, y, z 
    ...
    def setUpSimulation(self):
        ...
        vertexalpha = (xlen*.05 + self.dia,self.dia+1)
        vertexbeta =  (xlen*.9,.6*zlen)
    ...
    def fillCubeWithCBD2(self,x,y,z,x2,y2,z2):
        ...
        zl = min(z,z2)
        zh = max(z,z2)
        for yi in np.arange(yl,yh,self.sol_dia+0.7*(self.sol_dia)):
            for zi in np.arange(zl,zh,self.sol_dia):
                for xi in np.arange(xl,xh,self.sol_dia):
                    self.molID += 1
                    atom_type = self.solvent_type
                    self.appendLine(atom_type,xi,yi,zi)   
