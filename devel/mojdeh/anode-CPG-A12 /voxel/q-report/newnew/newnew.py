...
slice_horizontal = False
slice_verticalx = False
slice_verticaly = False
...
readinFile = "less_repel_30.lammpstrj"
...
with open(readinFile,"r") as inFile: 
	with open(outputFile, "w") as outFile:
		...
		zmax = max(zlist) + Aradius - resolution
		zmin = min(zlist) - Aradius + resolution
		print ("new zmax = ",zmax)
		print ("new zmin = ",zmin)
		...
		NX = math.floor((xlen - 0.2)/0.4) +1
		NY = math.floor((ylen - 0.2)/0.4) +1
		NZ = math.floor((zlen - 0.2)/0.4) +1
		...
		if slice_verticalx == True:
			xmax = xmax - 0.4*xlen
			xmin = xmin + 0.4*xlen

		if slice_verticaly == True:
			ymax = ymax - 0.4*ylen
			ymin = ymin + 0.4*ylen
