...
class DatafileGenerator():
            ...
            for zi in np.arange(zl--self.dia*5,zh-self.dia*7,self.dia*7):
                for xi in np.arange(xl+self.dia*3,xh-self.dia*2,self.dia*7):
                    ...
                    if val == 0:
                        ...
                        if value == 0:
                            self.drawDisc1(xi,yi,zi,radius)
                        elif value == 1:
                            self.drawDisc2(xi,yi,zi,radius)
                        else:
                            self.drawDisc3(xi,yi,zi,radius)
    ...
    def drawDisc(self,x,y,z,radius):
        ...
        for xi in np.arange(x-2*radius,x+2*radius,self.dia):
            for yi in np.arange(y-2*radius,y+2*radius,self.dia):
                for zi in np.arange(z-thickness,z+2*thickness,self.dia):
                    if ((xi-x)**2 + (yi-y)**2 + (zi-z)**2) < radius**1.9:
                        self.appendLine(self.active_type,xi,yi,zi)

    def drawDisc1(self,x,y,z,radius):
        self.molID += 1
        thickness = (self.dia/2)*1
        for xi in np.arange(x-2*radius,x+2*radius,self.dia):
            for yi in np.arange(y-2*radius,y+2*radius,self.dia):
    ...
    def drawDisc2(self,x,y,z,radius):
        self.molID += 1
        thickness = (self.dia/2)*1
        for xi in np.arange(x-thickness,x+2*thickness,self.dia):
            for yi in np.arange(y-2*radius,y+2*radius,self.dia):
                for zi in np.arange(z-2*radius,z+2*radius,self.dia):
                    if ((xi-x)**2 + (yi-y)**2 + (zi-z)**2) < radius**1.9:
                        self.appendLine(self.active_type,xi,yi,zi)
    def drawDisc3(self,x,y,z,radius):
        self.molID += 1
        thickness = (self.dia/2)*1
        for xi in np.arange(x-2*radius,x+2*radius,self.dia):
            for yi in np.arange(y-thickness,y+2*thickness,self.dia):
                for zi in np.arange(z-2*radius,z+2*radius,self.dia):
                    if ((xi-x)**2 + (yi-y)**2 + (zi-z)**2) < radius**1.9:
                        self.appendLine(self.active_type,xi,yi,zi)
