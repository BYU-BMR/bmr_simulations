...
class DatafileGenerator():
   ...
   # data string containing molecule ID, type, dia, rho, x, y, z, 0 0 0 
    ...
    def fillCubeWithRandomMix(self,x,y,z,x2,y2,z2):
        ...
        zl = min(z,z2)
        zh = max(z,z2)
        for yi in np.arange(yl+self.act_dia/2+self.cbd_dia/2,yh-self.act_dia/2-self.cbd_dia/2,self.act_dia*2):
            for zi in np.arange(zl+self.act_dia/2+self.cbd_dia/2,zh-self.act_dia/2-self.cbd_dia/2,self.act_dia*2):
                for xi in np.arange(xl+self.act_dia/2+self.cbd_dia/2,xh-self.act_dia/2-self.cbd_dia/2,self.act_dia*2):
                    val = random.randint(0,99)
                    if val >= 0 and val < 21:
                        atom_type = self.active_type
                        self.molID += 1
