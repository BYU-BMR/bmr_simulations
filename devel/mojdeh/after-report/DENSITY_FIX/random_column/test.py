...
class DatafileGenerator():
    ...
    def fillCubeWithRandomMix(self,x,y,z,x2,y2,z2):
        ...
        for yi in np.arange(yl+self.dia*3,yh-self.dia,self.dia*3):
            for zi in np.arange(zl--self.dia*5,zh-self.dia*7,self.dia*3):
                for xi in np.arange(xl+self.dia*3,xh-self.dia*2,self.dia*3):
                	xc = xi + random.randrange(-100,100)/100
                	yc = yi + random.randrange(-100,100)/100
                	zc = zi + random.randrange(-100,100)/100
                	val = random.randint(0,150)
                	if val == 0:
                		self.activecount += 1
                		self.drawRaspberry(xc,yc,zc,radius)
                	elif val >= 1 and val < 23:
                		self.cbdcount += 1
                		atom_type = self.cbd_type
                		self.appendLine(atom_type,xc+self.dia*5/2,yc+self.dia/2,zc+self.dia/2)
                	elif val >= 23 and val < 151:
                		self.solventcount += 1
                		atom_type = self.solvent_type
                		self.appendLine(atom_type,xc+self.dia*1/2,yc+self.dia/2,zc+self.dia/2)
