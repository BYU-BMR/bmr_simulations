...
solStr = "%d %d %d 0.01 0.0 1.0 %f %f %f\n" #string for solvent particles
...
for x in np.arange(x0+2.5*dia_cbd,x1-1,1.2*sol_dia):
    for y in np.arange(y0+1,y1-1,1.2*sol_dia):
        for z in np.arange(z0+1,z1-1,1.2*sol_dia):
