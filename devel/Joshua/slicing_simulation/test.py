...
class DatafileGenerator():
    ...
    def setUpSimulation(self):
        ...
        vertex_A = (0,zlen*0.5 + zlen*0.05/2 + self.dia/2)
        vertex_B = (xlen,zlen*0.05/2 + self.dia/2)
        ...
        self.fillCubeWithSolventVtxs(vertex_A,vertex_B)
        self.fillCubeWithCBDVtxs(vertex_A,vertex_B,checkForOverlap=True)
        ...
        for i in range(int(x_range)):
            for j in range(int(y_range)):
                for k in range(int(z_range)):
                    ...
                    self.drawSphere(x,y,x,radius)


    def fillCubeWithSolventVtxs(self,v1,v2):
        (x1,z1) = v1
        (x2,z2) = v2
        self.fillCubeWithSolvent(x1,self.y0,z1,x2,self.y1,z2)

    def fillCubeWithSolvent(self,x,y,z,x2,y2,z2):
        xl = min(x,x2)
        xh = max(x,x2)
        yl = min(y,y2)
        yh = max(y,y2)
                    ...
                    for k in range(int(z_range)):
                        ...
                        for line in activeParticleLines:
                            ...
                            if distance_sq < minDistance:
                                placeParticle = False
                                break
                        if (placeParticle) and val == 0 : self.appendLine(self.cbd_type,xi,yi,zi)
                        if (placeParticle) and val == 1 : self.appendLine(self.solvent_type,xi,yi,zi)
                        if (placeParticle) and val == 2 : self.appendLine(self.solvent_type,xi,yi,zi)
                        if (placeParticle) and val == 3 : self.appendLine(self.solvent_type,xi,yi,zi)
                        if (placeParticle) and val == 4 : self.appendLine(self.solvent_type,xi,yi,zi)
    ...
    def drawWall(self,x,z,x2,z2,atomType=wall_type):
        ...
        for i in range(numPoints):
            xi = x+i*delta_x
            zi = z+i*delta_z
            y1 = self.y1 - self.dia/2
            for yi in np.arange(self.y0,y1,self.dia*1):
